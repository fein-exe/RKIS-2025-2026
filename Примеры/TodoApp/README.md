...
Класс `TodoList` генерирует события при операциях с задачами:

- **ItemAdded** - срабатывает при добавлении новой задачи
- **ItemDeleted** - срабатывает при удалении задачи
- **ItemUpdated** - срабатывает при обновлении текста задачи
- **ItemStatusChanged** - срабатывает при изменении статуса задачи

События содержат информацию о задаче (`TodoItem`) и её индексе.

Пример использования в коде:

```csharp
var todoList = new TodoList();
todoList.ItemAdded += (sender, e) => Console.WriteLine($"Добавлена задача: {e.Item.Text}");
todoList.ItemDeleted += (sender, e) => Console.WriteLine($"Удалена задача с индексом {e.Index}");
```
...
Или используйте батник `build_project.bat` для сборки на Windows.
